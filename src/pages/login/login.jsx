...
function Login() {
  ...
  // FUncion para verificar si el usuario esta activo o no, esto unicamente lo puede hacer el admin.
  const verificarAccesoPermitido = async (uid) => {
    ...
    if (docSnap.exists()) {
      ...
      if (userData.activo === false) {
        await signOut(auth); // Lo expulsamos de Firebase Auth
  ...
  const handleSocialLogin = async (metodo) => {
    ...
    try {
      ...
      if (resultado.success) {
        ...
        //Primero se verifica si el el usuario esta activo o no
        const puedeEntrar = await verificarAccesoPermitido(user.uid);
        if (!puedeEntrar) {
          setLoading(false);
          return;
        }

        //  Si es nuevo, crearle el perfil
        ...
        if (!docSnap.exists()) {
          await setDoc(docRef, {
            ...
            activo: true // Los nuevos usuarios se crean activos activos
          });
        }
        navigate("/home");
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (resultado.success) {
      // si esta activo o no el usuarios
      const puedeEntrar = await verificarAccesoPermitido(resultado.user.uid);
      if (puedeEntrar) {
        navigate("/home");
      }
    } else {
